	...
	public static void main(String[] args) {
		...
		try {
			...
			for(FogDevice device : fogDevices){
				if(device.getName().startsWith("gateway")){ // names of all Smart Cameras start with 'm'
					moduleMapping.addModuleToDevice("motion_detector2", device.getName());
...
//			controller.setMobilityMap(mobilityMap);
	...
	private static FogDevice addArea(String id, int userId, String appId, int parentId){
		FogDevice router = createFogDevice("gateway-"+id, 1000, 10000, 100000, 100000, 1, 0.0, 107.339, 83.4333);
		fogDevices.add(router);
		router.setParentId(parentId);
		router.setUplinkLatency(0); // latency of connection between router and proxy server is 2 ms
		for(int i=0;i<numOfCamerasPerArea;i++){
			...
			camera.setUplinkLatency(0); // latency of connection between camera and router is 2 ms
...
//		camera.setParentId(parentId);
		...
		sensors.add(sensor);
		Sensor sensor2 = new Sensor("s2-"+id, "CAMERA2", userId, appId, new DeterministicDistribution(1)); // inter-transmission time of camera (sensor) follows a deterministic distribution
		sensors.add(sensor2);
		Actuator ptz = new Actuator("ptz-"+id, userId, appId, "PTZ_CONTROL");
		actuators.add(ptz);
		sensor.setGatewayDeviceId(parentId);
		sensor.setLatency(0.0);
		sensor2.setGatewayDeviceId(parentId);
		sensor2.setLatency(0.0);  // latency of connection between camera (sensor) and the parent Smart Camera is 1 ms
		ptz.setGatewayDeviceId(parentId);
		ptz.setLatency(0.0);  // latency of connection between PTZ Control and the parent Smart Camera is 1 ms
		return camera;
	}
...
//		}
		...
		try {
			fogdevice = new FogDevice(nodeName, characteristics, 
					new AppModuleAllocationPolicy(hostList), storageList, 0.5, upBw, downBw, 0, ratePerMips);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//		application.addAppModule("object_detector", 10);
		...
		application.addAppModule("motion_detector2", 10);
...
//		application.addAppModule("user_interface", 10);
		 ...
		 */   //todo
		application.addAppEdge("CAMERA2", "motion_detector2", 300, 100, "CAMERA2", Tuple.UP, AppEdge.SENSOR);
		application.addAppEdge("CAMERA", "motion_detector", 100, 100, "CAMERA", Tuple.UP, AppEdge.SENSOR); // adding edge from CAMERA (sensor) to Motion Detector module carrying tuples of type CAMERA
...
//		application.addAppEdge("object_detector", "object_tracker", 1000, 100, "OBJECT_LOCATION", Tuple.UP, AppEdge.MODULE); // adding edge from Object Detector to Object Tracker module carrying tuples of type OBJECT_LOCATION
		application.addAppEdge("motion_detector2", "PTZ_CONTROL", 28, 1000, "PTZ_PARAMS", Tuple.DOWN, AppEdge.ACTUATOR);
		application.addAppEdge("motion_detector", "PTZ_CONTROL", 28, 1000, "PTZ_PARAMS", Tuple.DOWN, AppEdge.ACTUATOR); // adding edge from Object Tracker to PTZ CONTROL (actuator) carrying tuples of type PTZ_PARAMS
		
		/*
		 * Defining the input-output relationships (represented by selectivity) of the application modules. 
		 */
		application.addTupleMapping("motion_detector2", "CAMERA2", "PTZ_PARAMS", new FractionalSelectivity(1));
		application.addTupleMapping("motion_detector", "CAMERA", "PTZ_PARAMS", new FractionalSelectivity(1)); // 1.0 tuples of type MOTION_VIDEO_STREAM are emitted by Motion Detector module per incoming tuple of type CAMERA
...
//		final AppLoop loop1 = new AppLoop(new ArrayList<String>(){{add("motion_detector");add("object_detector");add("object_tracker");}});
		final AppLoop loop2 = new AppLoop(new ArrayList<String>(){{add("CAMERA");add("motion_detector");add("PTZ_CONTROL");}});
		List<AppLoop> loops = new ArrayList<AppLoop>(){{add(loop2);}};
